"""
Highscore system read/write highscore elements from/to list
sort the list from moast points to lowest and displays the top 5
"""
...
def input_GUI():
    ...
    in_banner_msg = graph.Text(graph.Point(5, 7),
    ...
    while loop_ is True:
        ...
        if name_in.getText():
            ...
            empty_msg = graph.Text(graph.Point(5, 4), "Type youre name").draw(win)
...
def highscore_GUI(high_dict_list):
    ...
    text_pos_y = 8
    h_win = graph.GraphWin("Highscore", 500, 500)
    ...
    banner_msg = graph.Text(graph.Point(5, 9), "Highscore!").draw(h_win)
    banner_msg.setTextColor("White")
    banner_msg.setSize(20)

    if name:
        for i in range(0, len(name)):
            h_name_msg = graph.Text(graph.Point(4.5, text_pos_y), name[i] + ":").draw(h_win)
            h_points_msg = graph.Text(graph.Point(6, text_pos_y), points[i]).draw(h_win)
